...
多數自學規則得靠自己踩坑換來，但這一條在長壽 agent session 裡幾乎人人必踩，所以直接預埋：

> **Session 生命週期 ≠ 任務生命週期。** 為任務 A 開的 session 常常比任務 A 活得久 — 閒置一陣子後，又被拿來做任務 B。結果是：任務 B 蓋在任務 A 的分支上，或兩件不相干的工作混進同一個 commit。開始任何新任務前，先確認 (1) working tree 是乾淨的 (2) 當前分支對應這個任務。一個任務 = 一個分支 / 一個 commit 範圍；發現上個任務留下的未 commit 殘留，先處置完再開工。

**落地提示**：這個失敗發生在長 session 的後段 — 正是啟動時載入的規則被長 context 稀釋得最嚴重的時點，光靠 prompt 層提醒撐不住（與 Layer B 工程化保護是同一個教訓）。改用你 AI 工具的 per-interaction hook：每次使用者送出 prompt 時，注入 repo 狀態（未 commit 檔案、分支名、距上次 commit 的時間）加一段判斷指令，由 agent 比對殘留變更與本次要求 — 同一任務的延續就靜默繼續；不同任務就停手，先詢問使用者要不要把舊工作 commit / stash。處置決定權留在使用者手上，hook 永遠不自行 commit。
